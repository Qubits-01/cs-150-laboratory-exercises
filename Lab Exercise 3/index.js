...
class Section {
     ...
     */
    copyWith(completeName, dayBasedSchedules) {
        let dayBasedSchedsCopy = [];
        if (!dayBasedSchedules) {
            for (let sched of this._dayBasedSchedules) {
                dayBasedSchedsCopy.push(sched.copyWith());
            }
        }
        return new Section(completeName !== null && completeName !== void 0 ? completeName : this._completeName, dayBasedSchedules !== null && dayBasedSchedules !== void 0 ? dayBasedSchedules : dayBasedSchedsCopy);
    }
