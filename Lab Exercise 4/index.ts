...
function generalActionEffect(playerChoice: Choice): void {
    ...
    if (resultState === ResultState.PlayerWon) {
        playerScore++;
        if (playerScoreMsg !== null) {playerScoreMsg.textContent = `Player: ${playerScore}`;}
    } else if (resultState === ResultState.ComputerWon) {
        computerScore++;
        if (computerScoreMsg !== null) {computerScoreMsg.textContent = `Computer: ${computerScore}`;}
    }
    ...
    if (resultMsg !== null) {resultMsg.textContent = resultStateEnumToStringMsg(resultState);}

    // 6. Update round.
    round++;
    if (roundMsg !== null) {roundMsg.textContent = `${round}`;}
}

function pickRock(): void {
    // 1.1. Get player's choice.
    generalActionEffect(Choice.Rock);
}

function pickPaper(): void {
    // 1.1. Get player's choice.
    generalActionEffect(Choice.Paper);
}

function pickScissors(): void {
    // 1.1. Get player's choice.
...
function choiceEnumToString(choice: Choice): string {
    switch (choice) {
        case Choice.Rock:
            return "Rock";
        case Choice.Paper:
...
function intToChoiceEnum(choice: number): Choice {
    switch (choice) {
        case 0:
            return Choice.Rock;
        case 1:
...
function resultStateEnumToStringMsg(resultState: ResultState): string {
    switch (resultState) {
...
function whoWon(playerChoice: Choice, computerChoice: Choice): ResultState {
